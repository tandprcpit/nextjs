...
const SelectValue = ({ isOpen, isFocused, isDisabled, size, placeholder, placeholderIcon, ref }: SelectValueProps) => {
    return (
        <AriaButton
            ref={ref}
            className={cx(
                "relative flex cursor-pointer items-center rounded-lg  shadow-xs  outline-hidden transition duration-100 ease-linear ",
                (isFocused || isOpen) && "",
        ...
        >
            <AriaSelectValue<SelectItemType>
                className={cx(
                    "flex h-max items-center justify-start gap-2 truncate text-left align-middle",
                ...
                {(state) => {
                    ...
                    return (
                        <>
                            ...
                            {state.selectedItem ? (
                                <section className="flex gap-2 truncate">
                                    <p className="truncate text-md font-medium text-primary text-black">{state.selectedItem?.label}</p>
                ...
                {(state) => (
                    <>
                        ...
                        <Popover size={size} className={rest.popoverClassName}>
                            <AriaListBox items={items} className="size-full outline-hidden border-2 bg-white text-[#E64833] border-[#E64833]">
                                {children}
                            </AriaListBox>
                        </Popover>
