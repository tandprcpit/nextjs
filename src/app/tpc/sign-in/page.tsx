...
const Page = () => {
  ...
  const onSubmit = async (data: z.infer<typeof signInSchema>) => {
    ...
    const result = await signIn('tpc-credentials', { // Changed to tpc-credentials
      redirect: false,
    ...
    if (result?.url) {
      toast({
        title: 'Success',
        description: 'TPC login successfully.',
        variant: 'default',
      });
      router.replace('/tpc/dashboard'); // Updated URL for TPC dashboard
    }
  };
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center bg-[#FBE9D0]">
        <div className="max-w-md w-full space-y-8">
          <div className="bg-white p-8 rounded-lg shadow-xl">
            <h2 className="text-3xl font-bold mb-6 text-center text-[#244855]">TPC Sign In</h2> {/* Updated heading */}
